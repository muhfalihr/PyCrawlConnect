...
from controller.book.freetechbooks.allbook import All
from . import pk
...
@ns_api.route("/get-all-books-by-topic", methods=["GET"])
class GetAllBooksByTopic(Resource):
    @api.doc(
    ...
    def get(self):
        try:
            ...
            all = All()
            data = all.all(option="topics", page=page)
    ...
    @api.doc(
        ...
        params={
            "option": {
                ...
                "default": OptionsEnum.categories.value
            },
            "page": {
...
class GetBooksByID(Resource):
    ...
    def get(self):
        try:
            ...
            page = request.values.get("page")
            all = All()
            data = all.all(allbook=True, idlink=id, page=page)
            pk.produser(datas=data)
            return (
                success_response(data, message="success"), 200
            )
